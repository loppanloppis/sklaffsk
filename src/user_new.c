...
char *buf;
{
	...
	*tbuf = '\0';	/* Set 'length' of obuf */
	...
	sprintf(tmp, "%d:%d:%ld:%d:%s:%s:%d:%d:dum:dum:dum\n", ae->user, 
        	ae->pid, ae->login_time, ae->avail, ae->from, ae->tty,
		ae->action,ae->conf);
	strcpy(nbuf, obuf);
	strcat(nbuf, tmp);
	strcat(nbuf, buf);
...
/*
 * set_action - set action-flag in active-file
 * args: user (uid)
 * ret: yes (1) or no (0)
 */

int set_action(uid, act, cnf)
int uid, act, cnf;
{
...
struct USER_ENTRY *get_user_struct(uid)
int uid;
{
    int fd;
    char *buf, *oldbuf;
...
int setup_new_user()
{
    ...
    mkdir(Home, NEW_DIR_MODE);
    mkdir(Mbox, NEW_DIR_MODE);
...
char	*stringify_user_struct(user_entry, buf)
...
int add_active()
{
	...
	if (*tmp == MSG_YESANSWER) {
	  debuglog("user attempts to kill other process");
	    output(MSG_QUITTRY);
	    fflush(stdout);
	    kill(ae.pid, SIGTERM);
	    sleep(2L);
	    if (user_is_active(Uid)) {
	      debuglog("kill attempt failed, using SIGKILL");
		output("%s\n\n", MSG_QUITFAIL);
		output(MSG_KILLTRY);
		fflush(stdout);
		kill(ae.pid, SIGKILL);
    ...
    sprintf(tmp, "%d:%d:%ld:0:%s:%s:0:0:dum:dum:dum\n", Uid, getpid(), time(0),
	    hname, ttyname(0));
...
struct ACTIVE_ENTRY *check_active(uid, ae)
int uid;
struct ACTIVE_ENTRY *ae;
{
    int found;
...
struct UEN *sort_user(ue_list, count)
struct UEL *ue_list;
int count;
{
    int x;
	...
	if (fd != -1) {
		...
		for(;;) {
			...
			if (state == 6) {
			    ...
			    if (strcmp(headptr[i].heading,"muggen") == 0)
			            strcpy(kaffer->muggen, entry);
...
struct SKLAFFRC *kaffer;
{
    ...
    if (strlen(kaffer->muggen) > 0) {
	strcat(outbuf,"![muggen]\n");
	strcat(outbuf,kaffer->muggen);
	strcat(outbuf, "\n");
    }
